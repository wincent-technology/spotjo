...
class Chats extends React.Component {
  static navigationOptions = {
    ...
    renderItem = (item, index) => {
        ...
        onPress={() => this.props.navigation.navigate('ChatOneJob', { thread: item })}
      >
      ...
      checking = () => {
        try {
          http.POST('api/user/chat/companylist',{
            loginId : global.Id
          }).then(
            (res) => {
      ...
      cheks = (text) => {
        if (text != '') {
          ...
          if (newData != '') {
            this.setState({
              dataChecks: newData != null && newData,
              name: text,
            });
          } else {
            newData.push(text);
      ...
      >
       <View style={{marginTop:5,justifyContent:"center",alignItems:"center"}}>
        <View style={{flexDirection:'row',justifyContent:"center",alignItems:"center",width:wp(100),padding:7,}}>
            ...
            {this.timeConversion(item.createdAt)}
            </Text>
            </View>
            <Text style={{color:themeColor,fontSize:16,fontFamily:FontBold}}>
                {item.name}
    ...
    render() {
        const {
            data,add,show,suggesion,dataCheck,dataChecks
        } = this.state;
        let c = 0
        return (
        ...
        <NavigationEvents onDidFocus={this.checking}/>
                    ...
                    borderBottomColor:'#eee',paddingBottom:3,justifyContent:"center",alignItems:"center"}}>
                            <Text>
                            {dataCheck.reduce((tot, arr) => tot + arr.totalunRead,0)}  Message Unread
                            </Text>
                        </View>
                    <View style={{
...
export default withNavigationFocus(Chats);
