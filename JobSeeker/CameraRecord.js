...
import { Container, Button, Text, Icon, Footer, FooterTab, Spinner, H2, connectStyle, } from 'native-base';
...
import {getOrientation} from '../Component/orientation';
import KeyboardShiftView from '../Component/KeyboardShiftView';
import {runAfterInteractions} from '../Component/utils';
...
const styles = StyleSheet.create({
  ...
  capturingStyle: {
    ...
    bottom: 100,
    right:'40%',
    width: '20%',borderRadius:10,justifyContent:"center",alignItems:"center",
    ...
    if(cameraIds.length == 2){
      return (
        <Button
          ...
          selfCenter
        >
          <Icon transparent normal name='camera-reverse-outline' type='Ionicons'></Icon>
        </Button>
      )
    }
...
class Camera extends Component{
 ...
 constructor(props) {
  ...
  onRecordingStart = () => {
    ...
    // console.log('this>>',this._recordingTimer)
  ...
  render() {
    ...
    let buttons = (
      <View style={{backgroundColor:"#333",flex:1,justifyContent:"space-between",flexDirection:"row"}}>
        ...
        {recording ?
          <Button
            ...
            danger
          >
            <Icon name='video-slash' type='FontAwesome5' transparent></Icon>
          </Button>
          :
          <Button
            ...
            disabled={disable}
          >
            <Icon name='video' type='FontAwesome5' transparent></Icon>
          </Button>
        }
    ...
    let mainViewStyle = {
      ...
      height: orientation.height - (25 * isPortrait) - orientation.minusHeight - orientation.insetBottom,
      backgroundColor:"black"
    }
      ...
      <Container>
{
      ...
      {!this.state.show && <KeyboardShiftView style={styles.content} keyboardShouldPersistTaps={'never'} extraHeight={0} bounces={false}>
          <NavigationEvents
              ...
              >
                <React.Fragment>
                  ...
                  <View style={styles.buttonsView}>
                    {cameraCount <= 2 ?
                      <Button
                      textStyle={{color:"#afafaf"}}
                      style={{backgroundColor:"black"}}

                        onPress={this.changeWB}
                        selfCenter
                      >
                        <Text >{WB_OPTIONS_MAP[wb]}</Text>
                      </Button>
                    : null}
                    <Button
                      textStyle={{color:"#afafaf"}}
                      onPress={this.toggleRatio}
                      style={[styles.ratioButton],{backgroundColor:"black"}}
                      selfCenter
                    >
        ...
        {(takingPic || recording)?
              ...
              >
              {takingPic ? <H2 transparent>Capturing Picture...</H2> : 
              <Text style={{color:"white"}}>
              {`${audioDisabled ? ' (muted)' : ''} 00:${elapsed != -1 ? elapsed : "..."}`}
              </Text>
              }
              </View>
            : null}
...
maggi = async (file) => {
  ...
  resetZoom = () => {
    this._prevPinch = 1;TouchableOpacity
